...
// ─── Circuit breaker — 60s cooldown on 429, 24h on 402 ──────────────────────

const _cooldown: Record<string, number> = {}

function circuitIsOpen(label: string): boolean {
  const until = _cooldown[label]
  if (!until || Date.now() >= until) { delete _cooldown[label]; return false }
  return true
}

function circuitTrip(label: string, ms = 60_000) {
  _cooldown[label] = Date.now() + ms
  const until = new Date(_cooldown[label]).toLocaleTimeString("zh-TW", { timeZone: "Asia/Taipei" })
  console.warn(`[ai] ${label} rate-limited — cooldown ${ms / 1000}s, resumes ${until} Taipei`)
}
...
async function callProvider(
  ...
  if (!res.ok) {
    if (res.status === 429) circuitTrip(label)
    if (res.status === 402) circuitTrip(label, 24 * 60 * 60 * 1000)
...
// 13s gather window: take whatever responded within 13s; don't wait for stragglers.
...
export async function multiChat(userMsg: string): Promise<string> {
  ...
  await Promise.race([
    ...
    new Promise<void>(resolve => setTimeout(resolve, GATHER_MS)),
  ])
