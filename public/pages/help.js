...
[AI 操作摘要]</pre>
      ...
      <h4>強確認死叉（5 因子制）</h4>
      <p>死亡交叉通知會多一行確認評分，檢查五個空頭確認因子：</p>
      <ul>
        <li><strong>Vegas空頭排列</strong>：EMA144 &lt; EMA169</li>
        <li><strong>MACD柱轉負</strong>：動能已翻空</li>
        <li><strong>MA下彎</strong>：慢線 5 日斜率向下</li>
        <li><strong>RSI偏弱</strong>：RSI(14) 在 35–50（走弱但未超賣）</li>
        <li><strong>大盤走弱</strong>：市場指數（加密看 BTC、美股看 SPY、台股看 0050）收在 MA200 或以下</li>
      </ul>
      <p>五項全過顯示「⚡ 強確認死叉 5/5」— 回測（16 標的 × 約 9 年，含樣本外驗證）此類死叉 5 日內下跌機率約 83%，且精準度隨過關因子數單調上升（0 因子僅 36%）。未全過時顯示「死叉確認 X/5」與未過項目，供自行判斷。</p>
      <p style="color:var(--muted);font-size:0.85rem">注意：回測數據為歷史統計非保證；5/5 訊號極少見（回測中每年全清單約 2–3 次）。83% 統計僅在標的使用預設 MA25/60 時顯示——自訂均線組合的死叉未經此回測驗證，只顯示確認計數。黃金交叉不做此過濾——回測顯示金叉的價值在漲幅而非方向精準度，過濾會錯過主要獲利來源。</p>
      <h4>早安摘要</h4>
